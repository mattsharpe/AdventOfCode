...
namespace AdventOfCode.Solutions
{
    ...
    {
        public List<Day22Node> Nodes { get; set; } = new List<Day22Node>();
        public List<Tuple<Day22Node, Day22Node>> AdjacentPairs { get; set; }
        
        public override int GetHashCode()
        {
            var sb = new StringBuilder();
            
            Nodes.ForEach(x=> sb.AppendLine(x.ToString()));
            return sb.ToString().GetHashCode();
        }
        
        public void Print()
        {
            var goalNode = Nodes.Where(x => x.Y == 0).OrderByDescending(x => x.X).First();
            var maxX = Nodes.Max(x => x.X);
            var orderedNetwork = Nodes.OrderBy(x => x.Y).ThenBy(x => x.X);

            foreach (var node in orderedNetwork)
            {
                string output = " . ";
                if (node.NodeState == NodeState.Immovable)
                    output = " # ";
                if (node.X == goalNode.X && node.Y == goalNode.Y)
                    output = " G ";
                if (node.NodeState == NodeState.StartNode)
                    output = "(.)";
                if (node.NodeState == NodeState.Empty)
                    output = " _ ";
