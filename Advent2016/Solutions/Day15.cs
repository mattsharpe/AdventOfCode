...
namespace Advent2016.Solutions
{
    ...
    The sculpture is in a sealed enclosure and seems to involve a set of identical spherical capsules that are 
    carried to the top and allowed to bounce through the maze of spinning pieces.
    ...
    as it passes, the capsule bounces off the disc and soars away. You feel compelled to get one of those capsules.
    ...
    The second disc has ticked forward two positions at this point: it started at position 1, 
    ...
    */
    public class Day15
    {
        public List<Disc> Sculpture { get; set; } = new List<Disc>();


        public int Part1()
        {
            for (int t = 0; ; t++) // time counting up
            {
                if (Sculpture.TakeWhile((disc, depth) => (disc.StartingPosition + t + depth + 1) % disc.Positions == 0)
                    .Where((disc, depth) => depth == Sculpture.Count - 1).Any())
                {
                    return t;
                }
            }
